...
# Lac <-> Pyr
f1 <- reaction_data %>% filter(reactions == "Ser.hp->Pyr.Lvm") %>% pull(enzyme); f1
f2 <- reaction_data %>% filter(reactions == "Thr.hp->Pyr.Lvm") %>% pull(enzyme); f2
f3 <- reaction_data %>% filter(reactions == "Trp.hp->Pyr.Lvm") %>% pull(enzyme); f3




# f.all <- list(c(f1, f2), c(f5, f6), c(f9, f10), c(f11, f12))
f.all <- list(c(f1, f2, f3)) # more c(fx, fy, fz) can be included in the list()
...
func.calculate_confidence_interval <- function(f){
  ...
  while (binary.index <= total.binary.steps) { 
    ...
    for (optim.step in 1:n.opt.steps) { 
      ...
      for (tracer.k in tracers){
        message("sum of fluxes ", "v", f[1], "+v", f[2], "+v", f[3], "; stage ", stage, "; binary index ", binary.index, "; optim.step ", optim.step,   "; tracer ", tracer.k)
      ...
      Amat <- rbind(   Amat0[flux.C.I_index1, ] + Amat0[flux.C.I_index2, ] + Amat0[flux.C.I_index3, ]  , Amat)
      Amat <- rbind(-( Amat0[flux.C.I_index1, ] + Amat0[flux.C.I_index2, ] + Amat0[flux.C.I_index3, ] ), Amat)
      
      bo   <- c(   optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3]  + h,    bo)
      bo   <- c(-( optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3]  + h)-1, bo)     
      ...
      if  ((optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3] + h) < myMin) {
    ...
    d.flux.cost.h <- tibble(flux.index = paste0(flux.C.I_index1, "+", flux.C.I_index2, "+", flux.C.I_index3),
  ...
  save(d.flux.cost, 
       ...
       reaction_data,
       file = paste0("./CI/", 
                     flux.C.I_index1, "+", flux.C.I_index2, "+", flux.C.I_index3, ".RData"))
