...
filePath <- "LvM_fasted_TAGkinetics.xlsx"
...
load("../data/cleaned_labeling_data_Fasted_TAGkinetics.RData")
...
func.stoicEMU <- function(
  ...
  # myTracer= "13CLinoKinO" ;   tracerInputRate = 0
  # combine 
  ...
  # Special cases 
  # A1) add hepatic TAG TAGLino as substrate into the system
  # Note: without Lino.Blood->TAGLino in the network so these two inputs are decoupled
  # A2) Add TAGLino.W as lino's own unique substrate, critical for pulse-chase model, without affecting glycerol, palm and ole labeling (which is zero)
  substrate <- c(substrate, "TAGLino.Lv", "TAGLino.W")
  ...
  if (myTracer %>% str_detect("13CLinoKin")) {
    ...
    for (i in 1:length(l.EMU.substrates)){
      ...
      if ( str_detect(i_EMU, "TAGLino.W") ) {
        ...
        labeling.i <- d.13C.fasted.TAGkinetics %>%
...
# l.13CLinoKinP <- func.stoicEMU(myTracer = "13CLinoKinP", tracerInputRate = 0)
# l.13CLinoKinP <- func.stoicEMU(myTracer = "13CLinoKinP", tracerInputRate = 0)
# l.13CLinoKinQ <- func.stoicEMU(myTracer = "13CLinoKinQ", tracerInputRate = 0)
# l.13CLinoKinR <- func.stoicEMU(myTracer = "13CLinoKinR", tracerInputRate = 0)
# l.13CLinoKinS <- func.stoicEMU(myTracer = "13CLinoKinS", tracerInputRate = 0)
# l.13CLinoKinT <- func.stoicEMU(myTracer = "13CLinoKinT", tracerInputRate = 0)
