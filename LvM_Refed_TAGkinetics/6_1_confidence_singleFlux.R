...
if (state_fasted.or.refed == "refed") {
  ...
  # load the full set of data, but only use Tony's Cell Metab. data and linoleate tracing kinetics data
  # this is defined by 3_decompose_parallel where tracers used are specified
...
d.13C.refed_TAGkinetics.hpAA.SCFA %>% filter(Compound == "Lino")    %>% pull(Compound.tissue.seq_m.id) %>% unique()
d.13C.refed_TAGkinetics.hpAA.SCFA %>% filter(Compound == "Lino")    %>% pull(Compound.tissue.seq)      %>% unique()
d.13C.refed_TAGkinetics.hpAA.SCFA %>% filter(Compound == "TAGLino") %>% pull(Compound.tissue.seq_m.id) %>% unique()
...
func.calculate_confidence_interval <- function(f){
  ...
  # manual adjust: for Lino.Blood->AcCoA, it is quite special
  # we need larger step towards the right side to cover its C.I., as its optimal value is very close to zero, but has a very flat C.I. bottom
  # to make the code universally more robust, 'if ( round(flux.C.I_value.optimal) == 0) {' may be changed to < 10 (e.g., a small flux)
  # by here we make a specific patch for Lino.Blood->AcCoA to accelerate project speed
  if (flux.C.I_index == 10) {
    h0 <- 10
  }
  ...
  while (binary.index <= total.binary.steps) { 
    ...
    for (optim.step in 1:n.opt.steps) { 
      ...
      # this contraint should be removed
      # because srcAcCoA now includes FA/TAG and AAs and SCFAs
