...
func.cleanUp <- function(myExcel) {
  #check myExcel="/Users/boyuan/Desktop/Harvard/Research/Energy Metabolism Atlas/data/13C_Glycerol/20190401_13C_Glycerol_refed_xz387-388-389_tissue.xlsm"
...
d.13C_Glycerol <- tibble(.rows = 0)

for (a in dataFiles) {
  # check a=dataFiles[2]
  paste("working on file:", a) %>% print()
  d.13C_Glycerol <- bind_rows( d.13C_Glycerol, func.cleanUp(myExcel = a) )
}
...
# CHECK DATA CONTENT
d.13C_Glycerol$tissue %>% unique()
d.13C_Glycerol$Compound %>% unique()

# No duplicated tissues found. 

# Remove tissue suffix 
d.13C_Glycerol <- d.13C_Glycerol %>%
...
save(d.13C_Glycerol, file = "13C_Glycerol.RData")
