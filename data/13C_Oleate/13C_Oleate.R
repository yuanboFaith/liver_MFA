...
func.cleanUp <- function(myExcel) {
  #check myExcel="/Users/boyuan/Desktop/Harvard/Research/Energy Metabolism Atlas/data/13C_Oleate/20190401_13C_Oleate_refed_xz387-388-389_tissue.xlsm"
...
d.13C_Oleate <- tibble(.rows = 0)

for (a in dataFiles) {
  # check a=dataFiles[2]
  paste("working on file:", a) %>% print()
  d.13C_Oleate <- bind_rows( d.13C_Oleate, func.cleanUp(myExcel = a) )
}
...
# CHECK DATA CONTENT
d.13C_Oleate$tissue %>% unique()
d.13C_Oleate$Compound %>% unique()

# check data of duplicated serum
d.13C_Oleate %>% filter(str_detect(tissue, "SrTC")) %>% 
  filter(C_Label != 0) %>% 
  filter(Compound  %in% c("glucose", "lactate", "oleic acid" )) %>%
...
# The mice has both SrTC1 and 2, or SrTC4 alone, or SrTC1 alone. So remove SrTC2.
d.13C_Oleate <- d.13C_Oleate %>% filter(! tissue == "SrTC2" ) %>% 
...
save(d.13C_Oleate, file = "13C_Oleate.RData")


# load("13C_Oleate.RData")
